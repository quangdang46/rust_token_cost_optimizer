...
//! Raw output recovery -- saves unfiltered output to disk on command failure.
//!
//! ## Sensitive-output handling (port rtk#1988)
//!
...
//!    [`crate::redact::redact_content`] which masks Bearer tokens,
//!    `AKIA…` keys, `ghp_…` PATs, URL userinfo, and inline credential env
//!    assignments. If any matches were found, a `--- rtco: N credential-like
...
fn redaction_header(count: usize) -> String {
    format!(
        "--- rtco: {} credential-like patterns redacted ---\n",
        count
    )
}
...
// --- Sensitive variants (port rtk#1988) ---
//
...
// force the content redactor pass.
...
mod tests {
    ...
    /// Serialises tests that touch `RTCO_TEE*` / `RTCO_CONFIG_PATH` env vars so
...
"#;
    ...
    fn test_blocklisted_slug_skips_write() {
        ...
        // No file may exist for blocklisted slug.
    ...
    fn test_redactor_masks_bearer_in_content() {
        let tmpdir = tempfile::tempdir().expect("tempdir");
        // Synthetic Bearer token split via concat! so GitGuardian's Bearer-Token
        // detector does not match the fixture.
        let synth_token = concat!("abc123", "XYZdef456GHIjkl");
        let content = format!(
            "{}\nFailed request — Authorization: Bearer {}\n{}",
        ...
        assert!(
            !written.contains(synth_token),
            "raw bearer token must not survive: {}",
            written
        );
        assert!(
            written.starts_with("--- rtco: 1 credential-like patterns redacted ---\n"),
    ...
    fn test_tee_and_hint_sensitive_always_redacts() {
        ...
        // Synthetic Bearer token split via concat! to avoid GitGuardian flag.
        let synth_token = concat!("abc123", "XYZdef456GHIjkl");
        let content = format!(
        ...
        assert!(
            hint.starts_with("[full output saved — run: cat "),
            "hint shape: {hint}"
        );
    ...
    fn test_is_blocklisted_slug_examples() {
        // Positive: all sensitive prefixes from the design doc.
        for slug in [
            "aws_secretsmanager_get-secret-value",
            "aws_kms_decrypt",
            "aws_sts_get-session-token",
