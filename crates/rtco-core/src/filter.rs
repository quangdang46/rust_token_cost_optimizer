...
mod tests {
  ...
  "scripts": {
    ...
    fn test_minimal_filter_removes_comments() {
        // Pad with enough lines to exceed MIN_COMPRESS_BYTES (512).
        let mut code = String::from("// This is a comment\n");
        for i in 0..30 {
            code.push_str(&format!("fn dummy_{}() {{ /* padding */ }}\n", i));
        }
    ...
    #[test]
    fn test_small_input_skips_compression() {
        // Input under MIN_COMPRESS_BYTES is returned unchanged.
        let small = "// comment\nfn main() {}\n";
        assert!(small.len() < MIN_COMPRESS_BYTES);
        let filter = MinimalFilter;
        let result = filter.filter(small, &Language::Rust);
        assert_eq!(result, small, "Small input must pass through unchanged");
    }
