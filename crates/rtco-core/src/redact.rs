...
/// to prepend the `--- rtco: N credential-like patterns redacted ---` audit
/// header.
///
...
///
/// Used by the tracking DB so `rtco gain --by-project` can still distinguish
...
mod tests {
    ...
    fn test_redact_url_userinfo() {
        // Synthetic userinfo split via concat! so GitGuardian's Basic-Auth-URI
        // detector does not flag this fixture as a real secret.
        let input = concat!(
            "git push https://",
            "alice",
            ":",
            "s3cret",
            "@github.com/acme/repo.git main"
        );
    ...
    fn test_redact_flag_value_pairs() {
        // Synthetic flag values broken via concat! so the multi-flag string
        // does not match GitGuardian's Generic CLI Secret detector.
        let input = concat!(
            "tool --token=",
            "abc123",
            " --password ",
    ...
    fn test_redact_content_counts_substitutions() {
        // Synthetic credentials split via concat! so GitGuardian's Bearer Token
        // and Basic-Auth-URI detectors do not flag the fixture.
        let input = concat!(
            "log: GET https://",
            "alice",
            ":",
    ...
    fn test_redact_idempotent() {
        // Synthetic credential-shape inputs split via concat! so the file
        // does not trip GitGuardian's Basic-Auth-URI detector.
        ...
        let bearer_curl = concat!("curl -H 'Authorization: Bearer ", "abc", "' https://x");
        let inputs = [
            url_with_auth,
            bearer_curl,
