...
mod tests {
    ...
    fn test_single_dash_swallowed() {
        // rtco git diff -- file → clap gave ["file"], restore "--"
        let raw = vec!["rtco", "git", "diff", "--", "file"];
        let parsed = vec!["file"];
    ...
    fn test_args_before_dash() {
        // rtco cargo test name -- --nocapture → args before "--" stay before
        let raw = vec!["rtco", "cargo", "test", "name", "--", "--nocapture"];
    ...
    fn test_multiple_dashes_all_swallowed() {
        // rtco git diff -- -- -- → all 3 "--" swallowed, consecutive in output
        let raw = vec!["rtco", "git", "diff", "--", "--", "--"];
    ...
    fn test_dashes_with_args_between() {
        // rtco git diff -- arg1 -- arg2 → both "--" consumed, preserve positions
        let raw = vec!["rtco", "git", "diff", "--", "arg1", "--", "arg2"];
    ...
    fn test_multiple_dashes_some_preserved() {
        // rtco git diff -- -- → 2 in raw, 1 preserved in parsed
        let raw = vec!["rtco", "git", "diff", "--", "--"];
        let parsed = vec!["--"];
        assert_eq!(restore_with_raw(&parsed, &raw), vec!["--", "--"]);
    }
    ...
    fn test_dash_already_preserved() {
        // rtco cargo clippy -p pkg -- -D warnings → clap kept "--"
        let raw = vec![
            "rtco", "cargo", "clippy", "-p", "pkg", "--", "-D", "warnings",
        ];
    ...
    fn test_trailing_dash_preserved() {
        // rtco git diff file -- → trailing "--" preserved
        let raw = vec!["rtco", "git", "diff", "file", "--"];
        let parsed = vec!["file", "--"];
        assert_eq!(restore_with_raw(&parsed, &raw), vec!["file", "--"]);
    }
    ...
    fn test_no_dash_in_original() {
        let cases = vec![
            (
                vec!["rtco", "git", "diff", "feature/auth"],
                vec!["feature/auth"],
            ),
            (
                vec!["rtco", "git", "diff", "main...feature"],
                vec!["main...feature"],
            ),
            (vec!["rtco", "git", "diff", "main"], vec!["main"]),
            (
                vec!["rtco", "git", "diff", "--stat", "--cached"],
                vec!["--stat", "--cached"],
            ),
        ];
    ...
    fn test_duplicate_args_both_sides() {
        let raw = vec![
            "rtco", "cargo", "clippy", "-p", "p1", "-p", "p2", "--", "-p", "p3",
        ];
    ...
    fn test_cargo_clippy_missing_dash() {
        let raw = vec!["rtco", "cargo", "clippy", "-D", "warnings"];
    ...
    fn test_positional_equals_subcommand() {
        let raw = vec!["rtco", "git", "diff", "--", "diff"];
        let parsed = vec!["diff"];
    ...
    fn test_cargo_test_named_cargo() {
        let raw = vec!["rtco", "cargo", "test", "cargo", "--", "--nocapture"];
    ...
    fn test_git_diff_ref_before_path() {
        let raw = vec!["rtco", "git", "diff", "HEAD", "--", "file"];
    ...
    fn test_git_diff_flags_before_path() {
        let raw = vec!["rtco", "git", "diff", "--cached", "--", "file"];
    ...
    fn test_git_diff_multiple_files() {
        let raw = vec!["rtco", "git", "diff", "--", "file1", "file2", "file3"];
