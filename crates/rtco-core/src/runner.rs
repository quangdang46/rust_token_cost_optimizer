        ...
        RunMode::Streamed(filter) => {
            ...
            if let Some(label) = opts.tee_label {
                let hint = if opts.tee_sensitive {
                    crate::tee::tee_and_hint_sensitive(&result.raw, label, result.exit_code)
                } else {
                    crate::tee::tee_and_hint(&result.raw, label, result.exit_code)
                };
...
pub fn run_passthrough(tool: &str, args: &[std::ffi::OsString], verbose: u8) -> Result<i32> {
    ...
    let mut cmd = crate::utils::resolved_command(tool);
    cmd.args(args);
