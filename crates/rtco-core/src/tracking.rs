...
//!
//! ```no_run
//! use rtco_core::tracking::{TimedExecution, Tracker};
//!
...
/// flag and adds a one-shot `RTCO_TRACK=0` escape hatch mirroring the existing
/// `RTCO_TEE=0` pattern. Fails open: a missing/corrupt config never disables
/// tracking on its own.
fn tracking_enabled() -> bool {
    if std::env::var("RTCO_TRACK").ok().as_deref() == Some("0") {
        return false;
    }
...
///
/// ```no_run
/// use rtco_core::tracking::Tracker;
///
/// let tracker = Tracker::new()?;
/// tracker.record("ls -la", "rtco ls", 1000, 200, 50)?;
///
...
impl Tracker {
    ...
    fn open_path(db_path: PathBuf) -> Result<Self> {
        ...
        // half-broken DB never blocks `rtco gain` from opening.
    ...
    pub fn record(
        ...
        if !tracking_enabled() {
            return Ok(()); // silent no-op when [tracking] enabled = false / RTCO_TRACK=0
        }
    ...
    pub fn top_commands(&self, limit: usize) -> Result<Vec<String>> {
        ...
        let rows = stmt.query_map(params![limit as i64], |row| {
            ...
            // Extract just the command name (e.g. "rtco git status" → "git")
...
///
/// ```
/// use rtco_core::tracking::estimate_tokens;
///
...
impl TimedExecution {
    ...
    /// let timer = TimedExecution::start();
    /// // ... execute command ...
    /// timer.track("cmd", "rtco cmd", "input", "output");
    /// ```
    pub fn start() -> Self {
        Self {
            start: Instant::now(),
...
/// ```
/// use std::ffi::OsString;
/// use rtco_core::tracking::args_display;
///
...
mod tests {
    ...
    fn test_reset_all_clears_both_tables() {
        ...
        tracker
            .record(
                "git status",
                &format!("rtco git status reset_test_{}", pid),
                100,
                20,
                50,
            )
    ...
    // ── Finding 2 + 3 redaction & gating tests (port rtk#1987) ──
    //
    ...
    /// config file via `RTCO_CONFIG_PATH`. Avoids mutating `HOME` so we don't
    ...
    fn make_isolated_config_env(
        ...
        vec![
            ("RTCO_CONFIG_PATH", Some(p)),
            // Make sure no stray RTCO_TRACK from the host leaks in.
            ("RTCO_TRACK", None),
        ]
    }
    ...
    fn test_redact_command_credential_heuristic() {
        ...
        // they trip the prefix regex without flagging GitHub's
        // secret-scanner.
        let cases = [
    ...
    fn test_redact_command_idempotent() {
        // Synthetic credential split via concat! to avoid GitGuardian flag.
        let url_with_auth = concat!(
            "git push https://",
            "alice",
            ":",
            "s3cret",
    ...
    fn test_redact_project_path_shape() {
        let cases = [
            "/Users/alice/Projects/rtco",
            "/home/bob/work/secret-project",
            "C:\\Users\\carol\\code\\rtco",
            "/", // edge case
        ];
    ...
    fn test_migration_redacts_existing_rows() {
        ...
        // Userinfo split via concat! so GitGuardian's URI detector does not
        // flag the fixture.
        let original = concat!(
            "git push https://",
            "alice",
            ":",
            "s3cret",
            "@example.com/repo --token=ghp_ZZZZZZZZZZZZZZZZZZZZ"
        );
        tracker
            .conn
            .execute(
    ...
    fn test_tracking_disabled_via_config_skips_insert() {
        ...
        let before: i64 = tracker
            ...
            .expect("count");
        tracker
            .record("git status", "rtco git status", 100, 20, 5)
            .expect("record");
    ...
    fn test_tracking_disabled_via_env_skips_insert() {
        ...
        let after: i64 = tracker
            ...
            .query_row("SELECT COUNT(*) FROM commands", [], |r| r.get(0))
            .expect("count");
        assert_eq!(after, 0, "RTCO_TRACK=0 must skip the INSERT");
    }
    ...
    fn test_tracking_disabled_still_lets_reset_run() {
        ...
        tracker
            .record("git diff", "rtco git diff", 50, 10, 5)
            .expect("noop record");
