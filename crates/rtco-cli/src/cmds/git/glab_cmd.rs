...
use rtco_core::truncate::{CAP_LIST, CAP_WARNINGS};
...
static MR_URL_RE: LazyLock<Regex> = LazyLock::new(|| Regex::new(r"/-/merge_requests/(\d+)").unwrap());
/// Match GitLab CI section markers: section_start/end:timestamp:name[0K
static SECTION_MARKER_RE: LazyLock<Regex> =
    LazyLock::new(|| Regex::new(r"section_(?:start|end):\d+:[a-z0-9_]+(?:\x1b\[0K|\[0K)*").unwrap());
...
fn format_mr_list(json: &Value, ultra_compact: bool) -> String {
    ...
    if all_lines.len() > MAX_LIST {
        ...
        if let Some(hint) = rtco_core::tee::force_tee_tail_hint(&all_text, "glab-mrs", MAX_LIST + 1) {
            filtered.push_str(&format!("  {}\n", hint));
        }
    }
...
fn format_issue_list(json: &Value, ultra_compact: bool) -> String {
    ...
    if all_lines.len() > MAX_LIST {
        filtered.push_str(&format!("  … +{} more\n", all_lines.len() - MAX_LIST));
        let all_text = all_lines.join("\n");
        if let Some(hint) = rtco_core::tee::force_tee_tail_hint(&all_text, "glab-issues", MAX_LIST + 1) {
            filtered.push_str(&format!("  {}\n", hint));
        }
    }
...
fn format_ci_list(json: &Value, ultra_compact: bool) -> String {
    ...
    if all_lines.len() > MAX_CI_LIST {
        ...
        let all_text = all_lines.join("\n");
        if let Some(hint) =
            rtco_core::tee::force_tee_tail_hint(&all_text, "glab-pipelines", MAX_CI_LIST + 1)
        {
            filtered.push_str(&format!("  {}\n", hint));
        }
    }
