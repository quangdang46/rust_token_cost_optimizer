    ...
    if !prod.is_empty() {
        ...
        if cap && prod.len() > MAX_LISTING {
            ...
            let all_prod = prod
                ...
                .join("\n");
            if let Some(hint) =
                rtco_core::tee::force_tee_tail_hint(&all_prod, "pnpm-prod", MAX_LISTING + 1)
            {
    ...
    if !dev.is_empty() {
        ...
        if cap && dev.len() > MAX_LISTING {
            ...
            let all_dev = dev
                ...
                .join("\n");
            if let Some(hint) =
                rtco_core::tee::force_tee_tail_hint(&all_dev, "pnpm-dev", MAX_LISTING + 1)
            {
                lines.push(format!("  {}", hint));
            }
        }
...
pub fn run_passthrough(args: &[OsString], verbose: u8) -> Result<i32> {
    rtco_core::runner::run_passthrough("pnpm", args, verbose)
}
