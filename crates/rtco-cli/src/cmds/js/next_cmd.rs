...
fn filter_next_build(output: &str) -> String {
    ...
    // Unused-but-kept: route-pattern fallback for tier-2 degraded mode.
    #[allow(dead_code)]
    ...
    static BUNDLE_PATTERN: LazyLock<Regex> =
        LazyLock::new(|| Regex::new(r"^[○●◐λ✓]\s+([\w/\-\.]+)\s+(\d+(?:\.\d+)?)\s*(kB|B)\s+(\d+(?:\.\d+)?)\s*(kB|B)").unwrap());
...
fn extract_time(line: &str) -> Option<String> {
    static TIME_RE: LazyLock<Regex> = LazyLock::new(|| Regex::new(r"(\d+(?:\.\d+)?)\s*(s|ms)").unwrap());
