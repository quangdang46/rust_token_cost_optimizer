...
use rtco_core::utils::{package_manager_exec, strip_ansi};
use crate::parser::{
...
fn extract_stats_regex(output: &str) -> Option<TestResult> {
    // Unused-but-kept: tier-2 test-stats regex (degraded mode fallback).
    #[allow(dead_code)]
    static TEST_FILES_RE: LazyLock<Regex> =
        LazyLock::new(|| Regex::new(r"Test Files\s+(?:(\d+)\s+failed\s+\|\s+)?(\d+)\s+passed").unwrap());
...
pub fn run_test(command: &Commands, args: &[String], verbose: u8) -> Result<i32> {
    ...
    if let Some(hint) =
        rtco_core::tee::tee_and_hint(&combined, format!("{}_run", framework).as_str(), result.exit_code)
    {
        println!("{}\n{}", filtered, hint);
    } else {
        println!("{}", filtered);
