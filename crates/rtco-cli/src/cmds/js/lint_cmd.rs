...
use rtco_core::truncate::{CAP_ERRORS, CAP_WARNINGS};
use rtco_core::utils::{package_manager_exec, resolved_command, truncate};
use crate::mypy_cmd;
use crate::ruff_cmd;
...
pub fn run(args: &[String], verbose: u8) -> Result<i32> {
    ...
    if let Some(hint) = rtco_core::tee::tee_and_hint(&raw, "lint", result.exit_code) {
        println!("{}\n{}", filtered, hint);
    } else {
        println!("{}", filtered);
    }
...
fn filter_eslint_json(output: &str) -> String {
    ...
    if by_file.len() > MAX_FILES {
        ...
        let all_file_lines = by_file
            ...
            .join("\n");
        if let Some(hint) =
            rtco_core::tee::force_tee_tail_hint(&all_file_lines, "eslint-files", MAX_FILES + 1)
        {
            result.push_str(&format!("  {}\n", hint));
        }
    }
...
fn filter_pylint_json(output: &str) -> String {
    ...
    if file_counts.len() > MAX_FILES {
        ...
        let all_file_lines = file_counts
            ...
            .join("\n");
        if let Some(hint) =
            rtco_core::tee::force_tee_tail_hint(&all_file_lines, "pylint-files", MAX_FILES + 1)
        {
            result.push_str(&format!("  {}\n", hint));
        }
    }
...
fn filter_generic_lint(output: &str) -> String {
    ...
    if issues.len() > MAX_ISSUES {
        ...
        if let Some(hint) =
            rtco_core::tee::force_tee_tail_hint(&all_issues, "lint-issues", MAX_ISSUES + 1)
        {
            result.push_str(&format!("  {}\n", hint));
        }
    }
...
mod tests {
    ...
    fn test_detect_linter_after_npx_strip() {
        // Simulates: rtco lint npx eslint src/ → after strip_pm_prefix, args = ["eslint", "src/"]
