...
use rtco_core::truncate::{CAP_INVENTORY, CAP_LIST, CAP_WARNINGS};
...
fn format_kubectl_pods(json: &Value) -> String {
    ...
    if !issues.is_empty() {
        ...
        if issues.len() > MAX_PODS_ISSUES {
            ...
            if let Some(hint) =
                rtco_core::tee::force_tee_tail_hint(&all_issues, "kubectl-pods", MAX_PODS_ISSUES + 1)
            {
...
fn format_kubectl_services(json: &Value) -> String {
    ...
    if all_lines.len() > MAX_KUBECTL_SERVICES {
        ...
        if let Some(hint) =
            rtco_core::tee::force_tee_tail_hint(&all_text, "kubectl-services", MAX_KUBECTL_SERVICES + 1)
        {
            out.push_str(&format!(" {}", hint));
        }
        out.push('\n');
...
pub fn format_compose_ps(raw: &str) -> String {
    ...
    if all_formatted.len() > MAX_COMPOSE_SERVICES {
        ...
        if let Some(hint) = rtco_core::tee::force_tee_tail_hint(&all_text, "compose-ps", MAX_COMPOSE_SERVICES + 1) {
...
pub fn run_docker_passthrough(args: &[OsString], verbose: u8) -> Result<i32> {
    rtco_core::runner::run_passthrough("docker", args, verbose)
}
...
pub fn run_compose_passthrough(args: &[OsString], verbose: u8) -> Result<i32> {
    ...
    rtco_core::runner::run_passthrough("docker", &combined, verbose)
}
...
pub fn run_kubectl_passthrough(args: &[OsString], verbose: u8) -> Result<i32> {
    rtco_core::runner::run_passthrough("kubectl", args, verbose)
}
