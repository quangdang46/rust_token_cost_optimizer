...
use rtco_core::tee::force_tee_hint_sensitive;
use rtco_core::tracking;
use rtco_core::truncate::{CAP_INVENTORY, CAP_LIST};
use rtco_core::utils::{
...
pub fn run(subcommand: &str, args: &[String], verbose: u8) -> Result<i32> {
    ...
    match subcommand {
        ...
        "secretsmanager" if !args.is_empty() && args[0] == "get-secret-value" => {
            // Strip the rtco-private --reveal-secret flag before forwarding to aws CLI;
            // record it in REVEAL_SECRET_FLAG so filter_secrets_get can opt out of
            // redaction for this invocation. RTCO_REVEAL_SECRETS=1 has the same effect.
...
fn run_generic(subcommand: &str, args: &[String], verbose: u8, full_sub: &str) -> Result<i32> {
    ...
    if !output.status.success() {
        ...
        return Ok(rtco_core::utils::exit_code_from_output(&output, "aws"));
    }
...
fn run_aws_filtered(
    ...
    let rtco_label = format!("rtco {}", cmd_label);
    ...
    if !status.success() {
        ...
        if let Some(hint) = rtco_core::tee::tee_and_hint_sensitive(&raw, &slug, exit_code) {
            eprintln!("{}\n{}", stderr.trim(), hint);
        } else {
            eprintln!("{}", stderr.trim());
        }
        timer.track(&cmd_label, &rtco_label, &raw, &stderr);
        return Ok(exit_code);
    }
    ...
    if result.truncated {
        if let Some(hint) = rtco_core::tee::force_tee_hint_sensitive(&raw, &slug) {
            println!("{}\n{}", result.text, hint);
        } else {
            println!("{}", result.text);
        }
    } else if let Some(hint) = rtco_core::tee::tee_and_hint_sensitive(&raw, &slug, 0) {
        println!("{}\n{}", result.text, hint);
    } else {
        println!("{}", result.text);
    }

    timer.track(&cmd_label, &rtco_label, &raw, &result.text);
    Ok(0)
}
...
fn run_s3_ls(extra_args: &[String], verbose: u8) -> Result<i32> {
    ...
    if !output.status.success() {
        ...
        if let Some(hint) = rtco_core::tee::tee_and_hint_sensitive(&raw, "aws_s3_ls", exit_code) {
            eprintln!("{}\n{}", stderr.trim(), hint);
        } else {
            eprintln!("{}", stderr.trim());
        }
    ...
    if result.truncated {
        if let Some(hint) = rtco_core::tee::force_tee_hint_sensitive(&raw, "aws_s3_ls") {
            println!("{}\n{}", result.text, hint);
        } else {
            println!("{}", result.text);
        }
...
fn run_s3_transfer(operation: &str, extra_args: &[String], verbose: u8) -> Result<i32> {
    ...
    let rtco_label = format!("rtco aws s3 {}", operation);
...
fn filter_sqs_messages(json_str: &str) -> Option<FilterResult> {
    ...
    for msg in messages.iter().take(MAX_ITEMS) {
        ...
        let body_truncated = rtco_core::utils::truncate(body, 200);
...
/// Equivalent in effect to `RTCO_REVEAL_SECRETS=1`. Either trigger restores
...
fn should_reveal_secret() -> bool {
    ...
    matches!(
        std::env::var("RTCO_REVEAL_SECRETS").ok().as_deref(),
        Some("1")
    )
}
...
fn warn_redacted_once() {
    REDACTION_WARNED.call_once(|| {
        eprintln!("rtco: redacted SecretString (set RTCO_REVEAL_SECRETS=1 to disable)");
    });
}
...
mod tests {
    use super::*;
    use rtco_core::utils::count_tokens;
...
";
    ...
    // Serialises tests that touch RTCO_REVEAL_SECRETS or REVEAL_SECRET_FLAG so
    ...
    fn ensure_redaction_default() {
        REVEAL_SECRET_FLAG.store(false, Ordering::SeqCst);
        std::env::remove_var("RTCO_REVEAL_SECRETS");
    }
    ...
    fn test_filter_secrets_get_reveal_via_env() {
        ...
        // Opt-in: setting RTCO_REVEAL_SECRETS=1 restores the legacy behaviour.
        std::env::set_var("RTCO_REVEAL_SECRETS", "1");
    ...
    fn test_filter_secrets_get_token_savings() {
        ...
        // --output json` for an RDS credential bundle.
        //
        // Credential-shaped substrings are split via `concat!` so GitGuardian's
        // PostgreSQL-URI detector does not match the synthetic fixture as a real
        // secret. The runtime value of `raw` is identical to a single literal.
        let synth_pw = concat!("S3cret!", "Passw0rd-with-lots-of-entropy-9af2c1");
        let synth_host = concat!("prod-app.cluster-cxxxx.", "us-east-1.rds.amazonaws.com");
        let synth_user = concat!("app_", "user");
        let secret_string = format!(
            "{{\\\"engine\\\":\\\"postgres\\\",\\\"host\\\":\\\"{host}\\\",\
             \\\"username\\\":\\\"{user}\\\",\\\"password\\\":\\\"{pw}\\\",\
             \\\"dbname\\\":\\\"appdb\\\",\\\"port\\\":5432,\
             \\\"dbInstanceIdentifier\\\":\\\"prod-app\\\",\
             \\\"connection_url\\\":\\\"postgres://{user}:{pw}@{host}:5432/appdb?sslmode=require\\\"}}",
            user = synth_user,
            pw = synth_pw,
            host = synth_host,
        );
    ...
    "ResponseMetadata": {{
        ...
        // about token counts.
        assert!(!result.text.contains(synth_pw));
        assert!(!result.text.contains(synth_host));
    }
