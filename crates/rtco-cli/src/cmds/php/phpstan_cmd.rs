...
use rtco_core::utils::exit_code_from_status;
...
enum ErrorFormat {
    /// No format flag — rtco injects `--error-format=json`.
    Unspecified,
...
pub(crate) fn filter_phpstan_json(output: &str) -> String {
    ...
    let phpstan = match parsed {
        Ok(p) => p,
        Err(e) => {
            eprintln!("[rtco] phpstan: JSON parse failed ({})", e);
            return rtco_core::utils::fallback_tail(output, "phpstan (JSON parse error)", 5);
        }
    };
...
pub(crate) fn filter_phpstan_text(output: &str) -> String {
    ...
    rtco_core::utils::fallback_tail(output, "phpstan", 20)
}
