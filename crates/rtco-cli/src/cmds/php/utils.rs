use rtco_core::utils::{composer_tool_paths, resolve_binary, resolved_command};
use regex::Regex;
use std::path::Path;
...
static CONTROL_RE: LazyLock<Regex> = LazyLock::new(|| Regex::new(r"[\x00-\x08\x0B\x0C\x0E-\x1F\x7F]").unwrap());
