...
use rtco_core::truncate::CAP_LIST;
...
static BUILD_STATUS: LazyLock<Regex> = LazyLock::new(|| Regex::new(r"^BUILD (SUCCESSFUL|FAILED)").unwrap());
static ACTIONABLE: LazyLock<Regex> = LazyLock::new(|| Regex::new(r"^\d+ actionable tasks?").unwrap());
...
pub fn run(args: &[String], verbose: u8) -> Result<i32> {
    ...
    match detect_task(args) {
        GradlewTask::Build => runner::run_streamed(
            ...
            RunOptions::with_tee("gradlew_build").early_exit_on_failure(),
        ),
        GradlewTask::Test => runner::run_filtered(
            ...
            &args_display,
            filter_test,
            RunOptions::with_tee("gradlew_test").early_exit_on_failure(),
        ),
        GradlewTask::ConnectedTest => runner::run_filtered(
            ...
            &args_display,
            filter_connected,
            RunOptions::with_tee("gradlew_connected").early_exit_on_failure(),
        ),
        GradlewTask::Lint => runner::run_filtered(
            ...
            &args_display,
            filter_lint,
            RunOptions::with_tee("gradlew_lint").early_exit_on_failure(),
        ),
        GradlewTask::Dependencies => runner::run_filtered(
            ...
            &args_display,
            filter_dependencies,
            RunOptions::with_tee("gradlew_deps").early_exit_on_failure(),
        ),
        GradlewTask::Other => {
            let osargs: Vec<OsString> = args.iter().map(OsString::from).collect();
...
fn filter_build_line(line: &str) -> bool {
    static DAEMON_LINE: LazyLock<Regex> = LazyLock::new(|| {
        Regex::new(r"^(Starting a Gradle Daemon|Daemon will be stopped|Reusing configuration cache|Calculating task graph|> Configure project|Deprecated Gradle features|You can use|For more on this|Configuration cache entry)").unwrap()
    });
    static PROGRESS: LazyLock<Regex> =
        LazyLock::new(|| Regex::new(r"^\s*\d+%|^Downloading|^Configuring|^Resolving|^\[Incubating\]|^Wrote HTML report|^class \S+ could not|^\[android-").unwrap());
    static ERROR_LINE: LazyLock<Regex> = LazyLock::new(|| {
        Regex::new(r"(?i)(^FAILURE:|^\* What went wrong:|^\* Where:|> Could not|e: |error:|^Execution failed|Lint found \d+ error)").unwrap()
    });
    ...
    static BUILD_SCAN: LazyLock<Regex> = LazyLock::new(|| Regex::new(r"gradle\.com/s/|Publishing build scan").unwrap());
...
fn filter_test(output: &str) -> String {
    static FAILED_LINE: LazyLock<Regex> = LazyLock::new(|| Regex::new(r"FAILED$| FAILED ").unwrap());
    static PASSED_SKIPPED: LazyLock<Regex> = LazyLock::new(|| Regex::new(r" PASSED$| SKIPPED$").unwrap());
    static SUMMARY_LINE: LazyLock<Regex> =
        LazyLock::new(|| Regex::new(r"\d+ tests? completed|\d+ tests? failed|There were failing tests|See the report at").unwrap());
...
fn filter_connected(output: &str) -> String {
    ...
    static INSTRUMENTATION_RESULT: LazyLock<Regex> = LazyLock::new(|| Regex::new(r"^INSTRUMENTATION_RESULT:").unwrap());
    static INSTRUMENTATION_CODE: LazyLock<Regex> = LazyLock::new(|| Regex::new(r"^INSTRUMENTATION_CODE:").unwrap());
    static STARTING_TESTS: LazyLock<Regex> = LazyLock::new(|| Regex::new(r"^Starting \d+ tests? on ").unwrap());
    static INSTALLING_APK: LazyLock<Regex> = LazyLock::new(|| Regex::new(r"^Installing APK").unwrap());
...
fn filter_lint(output: &str) -> String {
    ...
    static REPORT_LINE: LazyLock<Regex> =
        LazyLock::new(|| Regex::new(r"Wrote (HTML|XML|text) report|file://|/build/reports/lint").unwrap());
