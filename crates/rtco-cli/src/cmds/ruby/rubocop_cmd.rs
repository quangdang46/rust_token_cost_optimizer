...
pub fn run(args: &[String], verbose: u8) -> Result<i32> {
    ...
    runner::run_filtered(
        ...
        runner::RunOptions::stdout_only().tee("rubocop").early_exit_on_failure(),
    )
}
...
fn filter_rubocop_json(output: &str) -> String {
    ...
    if files_with_offenses.len() > max_files {
        ...
        let all_files = files_with_offenses
            ...
            .join("\n");
        if let Some(hint) =
            rtco_core::tee::force_tee_tail_hint(&all_files, "rubocop-files", max_files + 1)
        {
            result.push_str(&format!("  {}\n", hint));
        }
    }
...
fn filter_rubocop_text(output: &str) -> String {
    ...
    rtco_core::utils::fallback_tail(output, "rubocop", 5)
}
