...
use rtco_core::utils::{fallback_tail, ruby_exec, truncate};
use anyhow::Result;
use regex::Regex;
...
static RE_SPRING: LazyLock<Regex> = LazyLock::new(|| Regex::new(r"(?i)running via spring preloader").unwrap());
static RE_SIMPLECOV: LazyLock<Regex> =
    LazyLock::new(|| Regex::new(r"(?i)(coverage report|simplecov|coverage/|\.simplecov|All Files.*Lines)").unwrap());
static RE_DEPRECATION: LazyLock<Regex> = LazyLock::new(|| Regex::new(r"^DEPRECATION WARNING:").unwrap());
static RE_FINISHED_IN: LazyLock<Regex> = LazyLock::new(|| Regex::new(r"^Finished in \d").unwrap());
static RE_SCREENSHOT: LazyLock<Regex> = LazyLock::new(|| Regex::new(r"saved screenshot to (.+)").unwrap());
static RE_RSPEC_SUMMARY: LazyLock<Regex> = LazyLock::new(|| Regex::new(r"(\d+) examples?, (\d+) failures?").unwrap());
...
fn filter_rspec_output(output: &str) -> String {
    ...
    match serde_json::from_str::<RspecOutput>(&stripped) {
        ...
        Err(e) => {
            eprintln!(
                "[rtco] rspec: JSON parse failed ({}), using text fallback",
                e
            );
        }
    }
