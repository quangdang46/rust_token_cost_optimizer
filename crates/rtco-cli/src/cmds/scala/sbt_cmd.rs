...
/// Entry point for `rtco sbt <args>` — dispatches to the task-specific
/// filter based on the first argument (test / compile / run / other).
pub fn run(args: &[String], verbose: u8) -> Result<i32> {
    let task = args.first().map(|s| s.as_str()).unwrap_or("test");
    if is_integration_test_cmd(task) || is_test_task(task) {
        return run_test(args, verbose);
    }
    match task {
        "test" | "testOnly" | "testQuick" => run_test(args, verbose),
        "compile" | "Compile" => run_compile(args, verbose),
        "run" | "Run" => run_run(args, verbose),
        _ => {
            // Scoped task (Test/test, exampleJVM/run) or passthrough: convert
            // String args to OsString for run_other's passthrough path.
            let os_args: Vec<OsString> = args.iter().map(OsString::from).collect();
            run_other(&os_args, verbose)
        }
    }
}
