...
fn build_pytest_summary(
    ...
    if !xfail_lines.is_empty() {
        ...
        if xfail_lines.len() > MAX_XFAIL {
            ...
            if let Some(hint) = rtco_core::tee::force_tee_tail_hint(&all_xfail, "pytest-xfail", MAX_XFAIL + 1) {
                result.push_str(&format!("  {}\n", hint));
            }
        }
    }
    ...
    if failures.len() > MAX_PYTEST_FAILURES {
        ...
        if let Some(hint) = rtco_core::tee::force_tee_hint(&all_failures, "pytest-failures") {
            result.push_str(&format!("  {}\n", hint));
        }
    }
