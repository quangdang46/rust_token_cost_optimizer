...
static PYTHON_FRAME_RE: LazyLock<Regex> = LazyLock::new(|| Regex::new(r#"^\s*File ".*", line \d+.*$"#).unwrap());
static PYTHON_EXCEPTION_RE: LazyLock<Regex> =
    LazyLock::new(|| Regex::new(r"^\s*[A-Za-z_][A-Za-z0-9_.]*(?:Error|Exception):").unwrap());
static JS_FRAME_RE: LazyLock<Regex> = LazyLock::new(|| Regex::new(r"^\s*at .+:\d+:\d+.*$").unwrap());
...
pub fn run(args: &[String], verbose: u8) -> Result<i32> {
    ...
    let rtco_cmd = display_command("rtco uv", &args_display);
    ...
    if args.first().map(String::as_str) != Some("run") {
        ...
        timer.track_passthrough(&original_cmd, &format!("{rtco_cmd} (passthrough)"));
...
fn collect_traceback_block(lines: &[&str], start_idx: usize) -> (Vec<String>, usize) {
    ...
    if frames.len() > MAX_TRACEBACK_FRAMES {
        ...
        if let Some(hint) = rtco_core::tee::force_tee_hint(&full_traceback, "uv-traceback") {
...
fn collect_error_block(lines: &[&str], start_idx: usize) -> (Vec<String>, usize) {
    ...
    if continuation_count > MAX_ERROR_CONTINUATION_LINES {
        ...
        if let Some(hint) = rtco_core::tee::force_tee_hint(&full_block, "uv-error-block") {
            block.push(format!("  {hint}"));
        }
    }
