...
static UNKNOWN_FLAG_RE: LazyLock<Regex> = LazyLock::new(|| {
    Regex::new(r"(?i)(unexpected argument|unknown (option|flag)|unrecognized (option|flag)|invalid (option|flag))").unwrap()
});
...
static MISSING_ARG_RE: LazyLock<Regex> = LazyLock::new(|| {
    Regex::new(r"(?i)(requires a value|requires an argument|missing (required )?argument|expected.*argument)").unwrap()
});
...
static USER_REJECTION_RE: LazyLock<Regex> = LazyLock::new(|| {
    Regex::new(r"(?i)(user (doesn't want|declined|rejected|cancelled)|operation (cancelled|aborted) by user)").unwrap()
});
