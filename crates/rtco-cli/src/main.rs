...
enum Commands {
    ...
    Read {
        ...
        level: rtco_core::filter::FilterLevel,
        /// Max lines
    ...
    /// PHP interpreter / artisan with compact output
    Php {
...
fn run_fallback(parse_error: clap::Error) -> Result<i32> {
    ...
    let error_message = rtco_core::utils::strip_ansi(&parse_error.to_string());
    ...
    if let Some(filter) = toml_match {
        ...
        let result = if filter.filter_stderr {
            ...
            rtco_core::utils::resolved_command(&args[0])
                .args(&args[1..])
        ...
        match result {
            Ok(output) => {
                let exit_code = rtco_core::utils::exit_code_from_output(&output, &raw_command);
                ...
                let tee_hint = if !output.status.success() {
                    rtco_core::tee::tee_and_hint(&combined_raw, &raw_command, exit_code)
                } else {
                    None
                };
                ...
                rtco_core::tracking::record_parse_failure_silent(
                    &raw_command,
                    &error_message,
                    true,
                );
        ...
        let status = rtco_core::utils::resolved_command(&args[0])
            .args(&args[1..])
            .stdin(std::process::Stdio::inherit())
        ...
        match status {
            Ok(s) => {
                ...
                Ok(rtco_core::utils::exit_code_from_status(&s, &raw_command))
            }
            Err(e) => {
                rtco_core::tracking::record_parse_failure_silent(
                    &raw_command,
...
fn main() {
    ...
    // e.g `rtco git log | head` exits silently instead of panicking.
    ...
    let code = match cli.command {
                    ...
                    if args.len() > 1 {
                        ...
                        match args[1].as_str() {
                            ...
                            _ => {
                                ...
                                let timer = rtco_core::tracking::TimedExecution::start();
                                let mut cmd = rtco_core::utils::resolved_command("npx");
                                for arg in &args {
                                    cmd.arg(arg);
                                }
                                ...
                                rtco_core::utils::exit_code_from_status(&status, "npx prisma")
                            }
                        }
                    } else {
                        let timer = rtco_core::tracking::TimedExecution::start();
                        let status = rtco_core::utils::resolved_command("npx")
                            .arg("prisma")
                            .status()
        ...
        Commands::Proxy { args } => {
            ...
            let mut child = ChildGuard(Some(
                rtco_core::utils::resolved_command(cmd_name.as_ref())
                    .args(&cmd_args)
                    .stdout(Stdio::piped())
                    .stderr(Stdio::piped())
                    .spawn()
            ...
            rtco_core::utils::exit_code_from_status(&status, &cmd_name)
        }
...
mod tests {
    ...
    fn test_broken_pipe_does_not_crash() {
        ...
        let status = child.wait().expect("Failed to wait for rtco");
        let code = status.code().unwrap_or(-1);

        assert_ne!(
            code, 134,
            "rtco crashed with SIGABRT (exit 134) on broken pipe - SIGPIPE handler missing"
        );
    }
