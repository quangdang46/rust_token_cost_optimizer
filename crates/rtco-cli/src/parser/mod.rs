...
//!
//! The three-tier system ensures rtco never returns false data silently.
...
pub trait OutputParser: Sized {
    ...
    /// 3. Return truncated passthrough with `[rtco:PASSTHROUGH]` marker
...
pub fn truncate_passthrough(output: &str) -> String {
    let max_chars = rtco_core::config::limits().passthrough_max_chars;
...
pub fn truncate_output(output: &str, max_chars: usize) -> String {
    ...
    let safe_max = if max_chars > chars.len() {
        chars.len()
    } else {
        max_chars
    };
    ...
    format!(
        "{}\n\n[rtco:PASSTHROUGH] Output truncated ({} chars → {} chars)",
        truncated,
        chars.len(),
        max_chars
    )
...
pub fn emit_degradation_warning(tool: &str, reason: &str) {
    eprintln!("[rtco:DEGRADED] {} parser: {}", tool, reason);
}

/// Helper to emit passthrough warning
pub fn emit_passthrough_warning(tool: &str, reason: &str) {
    eprintln!("[rtco:PASSTHROUGH] {} parser: {}", tool, reason);
}
...
mod tests {
    ...
    fn test_truncate_output() {
        ...
        assert!(truncated.contains("[rtco:PASSTHROUGH]"));
    ...
    fn test_truncate_output_multibyte() {
        ...
        assert!(result.contains("[rtco:PASSTHROUGH]"));
