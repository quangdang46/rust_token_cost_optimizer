...
//!
//! Combines ccusage (tokens spent) with rtco tracking (tokens saved) to provide
...
use rtco_core::utils::{format_cpt, format_tokens, format_usd};
...
pub struct PeriodEconomics {
    ...
    // rtco metrics
    pub rtco_commands: Option<usize>,
    pub rtco_saved_tokens: Option<usize>,
    pub rtco_savings_pct: Option<f64>,
...
impl PeriodEconomics {
    fn new(label: &str) -> Self {
        Self {
            ...
            rtco_commands: None,
            rtco_saved_tokens: None,
            rtco_savings_pct: None,
    ...
    fn set_rtco_from_day(&mut self, stats: &DayStats) {
        self.rtco_commands = Some(stats.commands);
        self.rtco_saved_tokens = Some(stats.saved_tokens);
        self.rtco_savings_pct = Some(stats.savings_pct);
    }

    fn set_rtco_from_week(&mut self, stats: &WeekStats) {
        self.rtco_commands = Some(stats.commands);
        self.rtco_saved_tokens = Some(stats.saved_tokens);
        self.rtco_savings_pct = Some(stats.savings_pct);
    }

    fn set_rtco_from_month(&mut self, stats: &MonthStats) {
        self.rtco_commands = Some(stats.commands);
        self.rtco_saved_tokens = Some(stats.saved_tokens);
        self.rtco_savings_pct = Some(if stats.input_tokens + stats.output_tokens > 0 {
    ...
    fn compute_weighted_metrics(&mut self) {
        ...
        if let (Some(cost), Some(saved)) = (self.cc_cost, self.rtco_saved_tokens) {
...
struct Totals {
    ...
    rtco_commands: usize,
    rtco_saved_tokens: usize,
    rtco_avg_savings_pct: f64,
...
fn merge_daily(cc: Option<Vec<CcusagePeriod>>, rtco: Vec<DayStats>) -> Vec<PeriodEconomics> {
...
fn merge_weekly(cc: Option<Vec<CcusagePeriod>>, rtco: Vec<WeekStats>) -> Vec<PeriodEconomics> {
    let mut map: HashMap<String, PeriodEconomics> = HashMap::new();
    ...
    // Merge rtco data (week_start = legacy Saturday "2026-01-18")
...
fn merge_monthly(cc: Option<Vec<CcusagePeriod>>, rtco: Vec<MonthStats>) -> Vec<PeriodEconomics> {
    let mut map: HashMap<String, PeriodEconomics> = HashMap::new();
...
fn compute_totals(periods: &[PeriodEconomics]) -> Totals {
    ...
    for p in periods {
        ...
        if let Some(cmds) = p.rtco_commands {
            totals.rtco_commands += cmds;
        }
        if let Some(saved) = p.rtco_saved_tokens {
            totals.rtco_saved_tokens += saved;
        }
        if let Some(pct) = p.rtco_savings_pct {
            pct_sum += pct;
            pct_count += 1;
        }
    }
    ...
    if pct_count > 0 {
        totals.rtco_avg_savings_pct = pct_sum / pct_count as f64;
    }
    ...
    if weighted_units > 0.0 {
        ...
        totals.savings_weighted = Some(totals.rtco_saved_tokens as f64 * input_cpt);
    }
    ...
    if totals.cc_total_tokens > 0 {
        ...
        totals.savings_blended =
            Some(totals.rtco_saved_tokens as f64 * totals.blended_cpt.unwrap());
    }
    if totals.cc_active_tokens > 0 {
        totals.active_cpt = Some(totals.cc_cost / totals.cc_active_tokens as f64);
        totals.savings_active = Some(totals.rtco_saved_tokens as f64 * totals.active_cpt.unwrap());
    }
...
fn display_summary(tracker: &Tracker, verbose: u8) -> Result<()> {
    ...
    println!("  rtco commands:                 {}", totals.rtco_commands);
    println!(
        "  Tokens saved:                 {}",
        format_tokens(totals.rtco_saved_tokens)
    );
    println!();
    ...
    println!("  rtco compresses CLI outputs before they enter Claude's context.");
...
fn print_period_table(periods: &[PeriodEconomics], verbose: u8) {
    ...
    if verbose > 0 {
        ...
        println!(
            ...
            "Period", "Spent", "Saved", "Savings", "Active$", "Blended$", "rtco Cmds"
        );
        println!(
            "{:-<12} {:-<10} {:-<10} {:-<10} {:-<10} {:-<12} {:-<12}",
            "", "", "", "", "", "", ""
        ...
        println!(
            "{:<12} {:>10} {:>10} {:>10} {:>12}",
            "Period", "Spent", "Saved", "Savings", "rtco Cmds"
        );
        println!(
            "{:-<12} {:-<10} {:-<10} {:-<10} {:-<12}",
            "", "", "", "", ""
...
fn export_csv(
    ...
    println!("period,spent,input_tokens,output_tokens,cache_create,cache_read,active_tokens,total_tokens,saved_tokens,weighted_savings,active_savings,blended_savings,rtco_commands");
...
fn print_csv_row(p: &PeriodEconomics) {
    ...
    let blended_savings = p
        ...
        .map(|s| format!("{:.4}", s))
        .unwrap_or_default();
    let cmds = p.rtco_commands.map(|c| c.to_string()).unwrap_or_default();
...
mod tests {
    ...
    fn test_period_economics_new() {
        ...
        assert!(p.rtco_commands.is_none());
    }
    ...
    fn test_merge_monthly_only_ccusage() {
        ...
        assert_eq!(merged.len(), 1);
        assert_eq!(merged[0].cc_cost, Some(12.34));
        assert!(merged[0].rtco_commands.is_none());
    }
    ...
    fn test_compute_weighted_input_cpt() {
        ...
        p.rtco_saved_tokens = Some(10_000);
    ...
    fn test_compute_totals() {
        let periods = vec![
            PeriodEconomics {
                ...
                rtco_commands: Some(5),
                rtco_saved_tokens: Some(2000),
                rtco_savings_pct: Some(50.0),
                weighted_input_cpt: None,
                savings_weighted: None,
                blended_cpt: None,
                active_cpt: None,
            ...
            PeriodEconomics {
                ...
                rtco_commands: Some(10),
                rtco_saved_tokens: Some(3000),
                rtco_savings_pct: Some(60.0),
                weighted_input_cpt: None,
                savings_weighted: None,
                blended_cpt: None,
                active_cpt: None,
        ...
        assert_eq!(totals.rtco_commands, 15);
        assert_eq!(totals.rtco_saved_tokens, 5000);
        assert_eq!(totals.rtco_avg_savings_pct, 55.0);
