...
### Auto-Configure Hooks in Every Detected Provider

The installer can install the `rtco-rewrite` hook into every AI provider
whose config file already exists on disk. This is opt-in — pass `--with-hooks`
to enable.

```bash
# Probe every known provider (Claude, Cursor, Cline, Windsurf, Copilot,
# OpenCode, Codex, Gemini, Amazon Q, Warp) and register hooks
