...
3. **Degradation warnings**: print `[RTCO:DEGRADED]` in verbose mode, `[RTCO:PASSTHROUGH]` on full fallback
...
```
[RTCO:DEGRADED] vitest parser: JSON parse failed at line 42, using regex fallback
[RTCO:PASSTHROUGH] playwright parser: Pattern mismatch, showing truncated output
```
